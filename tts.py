...
def downloader(url, query, downPath, numTry=10):
	params = {
		'ie': 'UTF-8',
		'q': query,
		'tl': LANG_MAP['english'],
...
def ask_usr():
	...
	#print(input_list)
	down_audio(url, input_list, down_path, save_path)


def down_audio(url, input_list, down_path, save_path):
	mkdir(down_path)
	count = 0
	pending = []
	...
	for sentence in input_list:#, desc='Downloading audio files', unit='sentance'):
		...
		for sub_sentence in sentence:
			thread = Thread(target=downloader, args=(url, sub_sentence, down_path + "data"+str(count)+".mp3"))
			...
			if len(pending) > 8:
				for thread in pending:
					thread.join()
				pending = []
			#downloader(url, sub_sentence, down_path + "data"+str(count)+".mp3")
			count += 1
	for thread in pending:
		thread.join()
